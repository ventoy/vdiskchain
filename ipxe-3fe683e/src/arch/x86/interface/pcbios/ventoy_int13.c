...
static __asmcall void int13 ( struct i386_all_regs *ix86 ) {
    ...
    //#if VTOY_DEBUG
    //printf("int13 0x%x 0x%x edx:0x%x\n", bios_drive, command, ix86->regs.edx); sleep(1);
    //#endif
...
static int int13_load_mbr ( unsigned int drive, struct segoff *address ) {
    ...
    copy_to_real(address->segment, address->offset, g_disk_buf_addr, 512);
 ...
 */
unsigned int ventoy_int13_hook (uint64_t disksize) 
{
	unsigned int blocks;
	unsigned int blocks_per_cyl;
	unsigned int natural_drive;
    ...
    /* a fake sandev */    
    g_sandev = (void *)umalloc(sizeof(struct san_device) + sizeof(struct int13_data));
    memset(g_sandev, 0, sizeof(struct san_device) + sizeof(struct int13_data));
    
    g_sandev->priv = int13 = (struct int13_data *)(g_sandev + 1);
    g_sandev->drive = int13->natural_drive = natural_drive;
    g_sandev->is_cdrom = 0;
    g_sandev->blksize_shift = 0;
    g_sandev->capacity.blksize = 512;
    g_sandev->capacity.blocks = disksize / 512;
    g_sandev->exdrive = 0;    
 ...
 */
int ventoy_int13_boot (unsigned int drive)
{
	int rc;
	struct segoff address;
